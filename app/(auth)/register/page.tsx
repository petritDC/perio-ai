...
export default function StaffRegisterPage() {
  ...
  const onSubmit = async (data: RegisterInput) => {
    ...
    const { error } = await supabase.auth.signUp({
      ...
      options: {
        data: { full_name: data.fullName, role: 'dentist' },
      },
    })
    if (error) {
      ...
      <div className="w-full max-w-md">
        <div className="flex items-center gap-3 justify-center mb-8">
          ...
          <div>
            ...
            <div className="text-[11px] text-slate-400 font-medium tracking-widest uppercase">Staff Portal</div>
          </div>
        </div>

        <div className="bg-white rounded-2xl p-8" style={{ boxShadow: 'var(--shadow-card)' }}>
          <h1 className="text-[22px] font-semibold text-slate-900 mb-1">Create account</h1>
          <p className="text-[14px] text-slate-500 mb-6">Register as clinic staff</p>

          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div>
              <Label htmlFor="fullName" className="text-[12px] text-slate-600 font-medium">Full Name</Label>
              <Input id="fullName" {...register('fullName')} className="mt-1" placeholder="Dr. Maya Chen" />
            ...
            <div>
              ...
              <Input id="email" type="email" {...register('email')} className="mt-1" placeholder="dr@clinic.com" />
